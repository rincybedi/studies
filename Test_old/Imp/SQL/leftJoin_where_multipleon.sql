exec sp_executesql N'SELECT 
    ...
    FROM ( SELECT DISTINCT 
        ...
        CASE WHEN ( EXISTS (SELECT 
            ...
            WHERE ([Project1].[SubjectId] = [Extent3].[SubjectId]) AND ([Extent3].[SchoolYearId] = @p__linq__3) AND ([Extent3].[ClassId] = @p__linq__4)
        ...
        FROM ( SELECT 
            ...
            INNER JOIN [dbo].[ClassSubject] AS [Extent2] ON ([Extent1].[SubjectId] = [Extent2].[SubjectId]) AND (@p__linq__0 = [Extent2].[ClassId])
            WHERE ([Extent2].[SchoolYearId] = @p__linq__1) AND ([Extent2].[ClassId] = @p__linq__2)
        )  AS [Project1]
    )  AS [Distinct1]',N'@p__linq__0 int,@p__linq__1 int,@p__linq__2 int,@p__linq__3 int,@p__linq__4 int',@p__linq__0=1,@p__linq__1=1,@p__linq__2=2,@p__linq__3=1,@p__linq__4=2


	   --using (var context = GetEdugenContext())
    --        {
    ...
    --                             //s.SubjectId equals cs.SubjectId into classSub
    --                             from CS in classSub.DefaultIfEmpty()
    --                             where CS.SchoolYearId == model.SchoolYearId && CS.ClassId == model.ClassId
